...
This function is taking the data stored in the compounddata file pushed from the code in the submit.jl file and creating a dictionary with it. this function then seperates the actual name of the molecule and its chemical formula and returns the formula for the parser file to read and encode. 
"""
function read_compounds_file(path::String)::Dict{String, MyChemicalCompoundModel}
    ...
    open(path, "r") do io # open a stream to the file
        for line in eachline(io) # read each line from the stream
            ...
            if (contains(line,"#") == false && contains(line, "name") == false)

                fields = split(line, ','); # splits around the ','

                # grab the fields -
                name = string(fields[1]);
                molecule = string(fields[2]);
                d = Dict{Char, Int64}()
                # build - 
                model = build(MyChemicalCompoundModel, name, molecule, d);
